...
public class MainMenuController {
    ...
    public void onCreditsButtonPress(Event evt) {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/breakoutgame/FXML/Credits.fxml"));
        try {
            Pane root = loader.load();
            ((Node)evt.getSource()).getScene().setRoot(root);
            ((CreditsController)loader.getController()).init(root);
        } catch (Exception e) {
            System.out.println("#In MainMenuController: " + getClass());
            System.out.println("#In MainMenuController: Fxml file loading error: " + e.getMessage());
        }
    }
